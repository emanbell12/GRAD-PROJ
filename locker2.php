...
<body>
    ...
    <div class="reservation-container">
        ...
        <form action="#" method="post" >
            ...
            // Query to select terms greater than or equal to today's date
            $sql = "SELECT * FROM availability WHERE FacilityID = $id AND DATE(Availability) >= '$today'";

            // Execute the query
            $result = mysqli_query($conn, $sql);

            echo '<h3>*Note: Reservations are made per Term</h3>';
            echo '<label for="date">Choose Term:</label>';
            echo '<select id="date" name="date" required>';
            while($row = mysqli_fetch_assoc($result)) {
                echo '<option value="' . $row['AvailabilityID'] . '" data-availabilityid="' . $row['AvailabilityID'] . '">' . date('Y-m-d', strtotime($row['Availability'])) . '</option>';
            }
            echo '</select>';
            ...
            echo '<label for="seat">Select Locker:</label>';
