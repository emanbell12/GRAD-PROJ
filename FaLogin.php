...
<body>
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $query = "SELECT id FROM users WHERE username = ? AND password = ? AND role = 'faculty'";
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            mysqli_close($conn);
            echo "<script>window.location.href = 'faservices.html?userId=$userId';</script>";
            exit;
        } else {
